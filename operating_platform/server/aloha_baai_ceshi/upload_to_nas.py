...
def local_server_request(api_url: str, data: Dict):
        """
        向本地服务器发送POST请求
...
def local_server_task_id_request(task_id: Union[str, int]):
    """
    发送任务ID到服务器
    ...
    local_server_request('api/upload_task_id', data)
...
def upload():
    if not nas_auth.get_auth_sid():
        ...
        return
    for i in range(7):
        date_data = get_date_offset(i)  # i=0 表示今天，i=1 表示昨天，...，i=6 表示 6 天前
        print(date_data)
                                    ...
                                    if task_part == "images":
                                        ...
                                        for camera_images in entries_2:
                                            ...
                                            if os.path.isdir(camera_images_path):
                                                img_list, video_list,label_video_path_list = get_img_video_path(each_task_path, camera_images, camera_images_path)
                                                ...
                                                if 'depth' in camera_images:
                                                    ...
                                                    if img_list:
                                                        for img_path, video_path in zip(img_list, video_list):
                                                            ...
                                                            if not encode_depth_video_frames(img_path, video_path, fps):
                                                    ...
                                                    if img_list:
                                                        for img_path, video_path in zip(img_list, video_list):
                                                            ...
                                                            if not encode_video_frames(img_path, video_path, fps):
                                                                ffmpeg_encode_flag = False
                                                                
                                                        for img_path, label_video_path in zip(img_list, label_video_path_list):
                                                            print(f"[INFO] 处理普通图像mp4: {img_path} -> {label_video_path}")
                                                            if not encode_label_video_frames(img_path, label_video_path, fps):
                                                                ffmpeg_encode_flag = False
                                        if ffmpeg_encode_flag:
                                            delete_directory(os.path.join(each_task_path, 'images'))
